...
class _SendEmailScreenState extends State<SendEmailScreen> {
  ...
  String? _validateField(String value, String label) {
    ...
    switch (label) {
      case 'To':
        if (trimmed.isEmpty) return AppStrings.recipientEmailRequired;
        if (!RegExp(r"^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$").hasMatch(trimmed)) {
          return AppStrings.invalidEmail;
        }
        break;
      case AppStrings.subject:
        if (trimmed.isEmpty) return AppStrings.subjectEmpty;
        break;
      case AppStrings.message:
        if (trimmed.isEmpty) return AppStrings.messageEmpty;
        break;
    }
    return null;
  }
