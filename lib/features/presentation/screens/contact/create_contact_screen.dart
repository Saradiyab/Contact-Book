...
import 'package:contact_app1/features/presentation/widgets/custom/custom_button.dart';
...
class _CreateContactScreenState extends State<CreateContactScreen> {
  ...
  void _submitForm() {
    if (_formKey.currentState!.validate()) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text(AppStrings.contactCreatedSuccess)));
                ...
                child: Column(
                  children: [
                    const Row(
                      ...
                      children: [
                        Text(AppStrings.contactDetails, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w400)),
                      ],
                    ),
                    ...
                    const Text(AppStrings.imageExtensionValidation, style: TextStyle(color: Colors.grey, fontSize: 18)),
                    const SizedBox(height: 8),
                    SizedBox(
                      width: 322,
                      height: 48,
                      child: ElevatedButton(
                        ...
                        child: const Text(AppStrings.uploadNewImage, style: TextStyle(color: Colors.white, fontSize: 16)),
                      ),
                    ),
                    ...
                    Form(
                      ...
                      child: Column(
                        children: [
                          Row(
                            children: [
                              Expanded(child: _buildInfoField(AppStrings.firstName, _firstNameController, keyboardType: TextInputType.name,)),
                              const SizedBox(width: 8),
                              Expanded(child: _buildInfoField(AppStrings.lastName, _lastNameController ,keyboardType: TextInputType.name,)),
                            ],
                          ),
                             const SizedBox(height: 10),
                          _buildInfoField(AppStrings.email1, _emailController, keyboardType: TextInputType.emailAddress,),
                             const SizedBox(height: 10),
                          _buildInfoField(AppStrings.phonestar, _phoneController, keyboardType: TextInputType.phone, ),
                             const SizedBox(height: 10),
                          _buildInfoField(AppStrings.email2, _email2Controller, keyboardType: TextInputType.emailAddress,),
                             const SizedBox(height: 10),
                          _buildInfoField(AppStrings.mobile, _mobileController,  keyboardType: TextInputType.number,),
                             const SizedBox(height: 10),
                          _buildInfoField(AppStrings.address, _addressController, keyboardType: TextInputType.text,),
                             const SizedBox(height: 10),
                          _buildInfoField(AppStrings.address2, _address2Controller,  keyboardType: TextInputType.text),
                          
                          
                          const SizedBox(height: 20),
                          _buildButton(AppStrings.cancel, AppColors.blue, _submitForm, backgroundColor: AppColors.blue),
                          const SizedBox(height: 10),
                          _buildButton(AppStrings.back, AppColors.blue, () {
                            Navigator.pushReplacement(
                              context,
                              MaterialPageRoute(builder: (context) => ContactsScreen(userToken: widget.userToken)),
                            );
